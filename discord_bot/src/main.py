...
# delete message 刪除指定則數的訊息
@bot.command(help = "delete message", brief = "delete message")
async def delete(ctx, limit=None):
    try:
        if limit is None:
            await ctx.send("NO given limit")
            return
        await ctx.message.channel.purge(limit = int(limit)+1)
        await ctx.send(f"```已經刪除了{limit}則訊息```")
    except Exception as e:
        await ctx.send(e)
